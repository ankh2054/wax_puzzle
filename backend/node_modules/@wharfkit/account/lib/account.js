'use strict';

var antelope = require('@wharfkit/antelope');
var resources = require('@wharfkit/resources');
var token = require('@wharfkit/token');
var tslib = require('tslib');
var contract = require('@wharfkit/contract');

var Permission_1;
exports.LinkedAction = class LinkedAction extends antelope.Struct {
};
tslib.__decorate([
    antelope.Struct.field('name')
], exports.LinkedAction.prototype, "account", void 0);
tslib.__decorate([
    antelope.Struct.field('name', { optional: true })
], exports.LinkedAction.prototype, "action", void 0);
exports.LinkedAction = tslib.__decorate([
    antelope.Struct.type('linked_actions')
], exports.LinkedAction);
exports.Permission = Permission_1 = class Permission extends antelope.Struct {
    static from(value) {
        if (antelope.isInstanceOf(value, Permission_1)) {
            return value;
        }
        return super.from(value);
    }
    get name() {
        return this.perm_name;
    }
    addKey(key, weight = 1) {
        const exists = this.required_auth.keys.find((k) => antelope.PublicKey.from(key).equals(k.key));
        if (exists) {
            throw new Error(`The provided key (${String(key)}) already exists on the "${this.perm_name}" permission.`);
        }
        this.required_auth.keys.push(antelope.KeyWeight.from({
            key: key,
            weight: weight,
        }));
        // Always sort authorities, required by antelopeio/leap
        this.required_auth.sort();
    }
    removeKey(key) {
        const index = this.required_auth.keys.findIndex((k) => antelope.PublicKey.from(key).equals(k.key));
        if (index === -1) {
            throw new Error(`The provided key (${String(key)}) does not exist on the "${this.perm_name}" permission.`);
        }
        this.required_auth.keys.splice(index, 1);
    }
    addAccount(permissionLevel, weight = 1) {
        const exists = this.required_auth.accounts.find((k) => antelope.PermissionLevel.from(permissionLevel).equals(k.permission));
        if (exists) {
            throw new Error(`The provided account (${String(antelope.PermissionLevel.from(permissionLevel))}) already exists on the "${this.perm_name}" permission.`);
        }
        this.required_auth.accounts.push(antelope.PermissionLevelWeight.from({
            permission: antelope.PermissionLevel.from(permissionLevel),
            weight: weight,
        }));
        // Always sort authorities, required by antelopeio/leap
        this.required_auth.sort();
    }
    removeAccount(permissionLevel) {
        const index = this.required_auth.accounts.findIndex((a) => antelope.PermissionLevel.from(permissionLevel).equals(a.permission));
        if (index === -1) {
            throw new Error(`The provided permission (${String(permissionLevel)}) does not exist on the "${this.perm_name}" permission.`);
        }
        this.required_auth.accounts.splice(index, 1);
    }
    addWait(wait) {
        this.required_auth.waits.push(antelope.WaitWeight.from(wait));
        // Always sort authorities, required by antelopeio/leap
        this.required_auth.sort();
    }
    removeWait(wait) {
        this.required_auth.waits = this.required_auth.waits.filter((w) => !antelope.WaitWeight.from(wait).equals(w));
    }
};
tslib.__decorate([
    antelope.Struct.field('name')
], exports.Permission.prototype, "perm_name", void 0);
tslib.__decorate([
    antelope.Struct.field('name')
], exports.Permission.prototype, "parent", void 0);
tslib.__decorate([
    antelope.Struct.field(antelope.Authority)
], exports.Permission.prototype, "required_auth", void 0);
tslib.__decorate([
    antelope.Struct.field(exports.LinkedAction, { array: true, optional: true })
], exports.Permission.prototype, "linked_actions", void 0);
exports.Permission = Permission_1 = tslib.__decorate([
    antelope.Struct.type('permission')
], exports.Permission);

const abiBlob = antelope.Blob.from('');
const abi = antelope.ABI.from(abiBlob);
class Contract extends contract.Contract {
    constructor(args) {
        super({
            client: args.client,
            abi: abi,
            account: antelope.Name.from('eosio'),
        });
    }
    action(name, data, options) {
        return super.action(name, data, options);
    }
    table(name, scope) {
        return super.table(name, scope, TableMap[name]);
    }
}
var Types;
(function (Types) {
    let AbiHash = class AbiHash extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], AbiHash.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], AbiHash.prototype, "hash", void 0);
    AbiHash = tslib.__decorate([
        antelope.Struct.type('abi_hash')
    ], AbiHash);
    Types.AbiHash = AbiHash;
    let Activate = class Activate extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], Activate.prototype, "feature_digest", void 0);
    Activate = tslib.__decorate([
        antelope.Struct.type('activate')
    ], Activate);
    Types.Activate = Activate;
    let KeyWeight = class KeyWeight extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.PublicKey)
    ], KeyWeight.prototype, "key", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], KeyWeight.prototype, "weight", void 0);
    KeyWeight = tslib.__decorate([
        antelope.Struct.type('key_weight')
    ], KeyWeight);
    Types.KeyWeight = KeyWeight;
    let PermissionLevel = class PermissionLevel extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], PermissionLevel.prototype, "actor", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], PermissionLevel.prototype, "permission", void 0);
    PermissionLevel = tslib.__decorate([
        antelope.Struct.type('permission_level')
    ], PermissionLevel);
    Types.PermissionLevel = PermissionLevel;
    let PermissionLevelWeight = class PermissionLevelWeight extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(PermissionLevel)
    ], PermissionLevelWeight.prototype, "permission", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], PermissionLevelWeight.prototype, "weight", void 0);
    PermissionLevelWeight = tslib.__decorate([
        antelope.Struct.type('permission_level_weight')
    ], PermissionLevelWeight);
    Types.PermissionLevelWeight = PermissionLevelWeight;
    let WaitWeight = class WaitWeight extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], WaitWeight.prototype, "wait_sec", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], WaitWeight.prototype, "weight", void 0);
    WaitWeight = tslib.__decorate([
        antelope.Struct.type('wait_weight')
    ], WaitWeight);
    Types.WaitWeight = WaitWeight;
    let Authority = class Authority extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], Authority.prototype, "threshold", void 0);
    tslib.__decorate([
        antelope.Struct.field(KeyWeight, { array: true })
    ], Authority.prototype, "keys", void 0);
    tslib.__decorate([
        antelope.Struct.field(PermissionLevelWeight, { array: true })
    ], Authority.prototype, "accounts", void 0);
    tslib.__decorate([
        antelope.Struct.field(WaitWeight, { array: true })
    ], Authority.prototype, "waits", void 0);
    Authority = tslib.__decorate([
        antelope.Struct.type('authority')
    ], Authority);
    Types.Authority = Authority;
    let BidRefund = class BidRefund extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], BidRefund.prototype, "bidder", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], BidRefund.prototype, "amount", void 0);
    BidRefund = tslib.__decorate([
        antelope.Struct.type('bid_refund')
    ], BidRefund);
    Types.BidRefund = BidRefund;
    let Bidname = class Bidname extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Bidname.prototype, "bidder", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Bidname.prototype, "newname", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Bidname.prototype, "bid", void 0);
    Bidname = tslib.__decorate([
        antelope.Struct.type('bidname')
    ], Bidname);
    Types.Bidname = Bidname;
    let Bidrefund = class Bidrefund extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Bidrefund.prototype, "bidder", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Bidrefund.prototype, "newname", void 0);
    Bidrefund = tslib.__decorate([
        antelope.Struct.type('bidrefund')
    ], Bidrefund);
    Types.Bidrefund = Bidrefund;
    let ProducerKey = class ProducerKey extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], ProducerKey.prototype, "producer_name", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.PublicKey)
    ], ProducerKey.prototype, "block_signing_key", void 0);
    ProducerKey = tslib.__decorate([
        antelope.Struct.type('producer_key')
    ], ProducerKey);
    Types.ProducerKey = ProducerKey;
    let ProducerSchedule = class ProducerSchedule extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], ProducerSchedule.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(ProducerKey, { array: true })
    ], ProducerSchedule.prototype, "producers", void 0);
    ProducerSchedule = tslib.__decorate([
        antelope.Struct.type('producer_schedule')
    ], ProducerSchedule);
    Types.ProducerSchedule = ProducerSchedule;
    let BlockHeader = class BlockHeader extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockHeader.prototype, "timestamp", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], BlockHeader.prototype, "producer", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], BlockHeader.prototype, "confirmed", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], BlockHeader.prototype, "previous", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], BlockHeader.prototype, "transaction_mroot", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], BlockHeader.prototype, "action_mroot", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockHeader.prototype, "schedule_version", void 0);
    tslib.__decorate([
        antelope.Struct.field(ProducerSchedule, { optional: true })
    ], BlockHeader.prototype, "new_producers", void 0);
    BlockHeader = tslib.__decorate([
        antelope.Struct.type('block_header')
    ], BlockHeader);
    Types.BlockHeader = BlockHeader;
    let BlockInfoRecord = class BlockInfoRecord extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], BlockInfoRecord.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockInfoRecord.prototype, "block_height", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], BlockInfoRecord.prototype, "block_timestamp", void 0);
    BlockInfoRecord = tslib.__decorate([
        antelope.Struct.type('block_info_record')
    ], BlockInfoRecord);
    Types.BlockInfoRecord = BlockInfoRecord;
    let BlockSigningAuthorityV0 = class BlockSigningAuthorityV0 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockSigningAuthorityV0.prototype, "threshold", void 0);
    tslib.__decorate([
        antelope.Struct.field(KeyWeight, { array: true })
    ], BlockSigningAuthorityV0.prototype, "keys", void 0);
    BlockSigningAuthorityV0 = tslib.__decorate([
        antelope.Struct.type('block_signing_authority_v0')
    ], BlockSigningAuthorityV0);
    Types.BlockSigningAuthorityV0 = BlockSigningAuthorityV0;
    let BlockchainParameters = class BlockchainParameters extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], BlockchainParameters.prototype, "max_block_net_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "target_block_net_usage_pct", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_transaction_net_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "base_per_transaction_net_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "net_usage_leeway", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "context_free_discount_net_usage_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "context_free_discount_net_usage_den", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_block_cpu_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "target_block_cpu_usage_pct", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_transaction_cpu_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "min_transaction_cpu_usage", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_transaction_lifetime", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "deferred_trx_expiration_window", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_transaction_delay", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], BlockchainParameters.prototype, "max_inline_action_size", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], BlockchainParameters.prototype, "max_inline_action_depth", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], BlockchainParameters.prototype, "max_authority_depth", void 0);
    BlockchainParameters = tslib.__decorate([
        antelope.Struct.type('blockchain_parameters')
    ], BlockchainParameters);
    Types.BlockchainParameters = BlockchainParameters;
    let BlockchainParametersV1 = class BlockchainParametersV1 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32, { optional: true })
    ], BlockchainParametersV1.prototype, "max_action_return_value_size", void 0);
    BlockchainParametersV1 = tslib.__decorate([
        antelope.Struct.type('blockchain_parameters_v1')
    ], BlockchainParametersV1);
    Types.BlockchainParametersV1 = BlockchainParametersV1;
    let Buyram = class Buyram extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Buyram.prototype, "payer", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Buyram.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Buyram.prototype, "quant", void 0);
    Buyram = tslib.__decorate([
        antelope.Struct.type('buyram')
    ], Buyram);
    Types.Buyram = Buyram;
    let Buyrambytes = class Buyrambytes extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Buyrambytes.prototype, "payer", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Buyrambytes.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], Buyrambytes.prototype, "bytes", void 0);
    Buyrambytes = tslib.__decorate([
        antelope.Struct.type('buyrambytes')
    ], Buyrambytes);
    Types.Buyrambytes = Buyrambytes;
    let Buyrex = class Buyrex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Buyrex.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Buyrex.prototype, "amount", void 0);
    Buyrex = tslib.__decorate([
        antelope.Struct.type('buyrex')
    ], Buyrex);
    Types.Buyrex = Buyrex;
    let Canceldelay = class Canceldelay extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(PermissionLevel)
    ], Canceldelay.prototype, "canceling_auth", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Checksum256)
    ], Canceldelay.prototype, "trx_id", void 0);
    Canceldelay = tslib.__decorate([
        antelope.Struct.type('canceldelay')
    ], Canceldelay);
    Types.Canceldelay = Canceldelay;
    let PowerupConfigResource = class PowerupConfigResource extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], PowerupConfigResource.prototype, "current_weight_ratio", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], PowerupConfigResource.prototype, "target_weight_ratio", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], PowerupConfigResource.prototype, "assumed_stake_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec, { optional: true })
    ], PowerupConfigResource.prototype, "target_timestamp", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64, { optional: true })
    ], PowerupConfigResource.prototype, "exponent", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32, { optional: true })
    ], PowerupConfigResource.prototype, "decay_secs", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset, { optional: true })
    ], PowerupConfigResource.prototype, "min_price", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset, { optional: true })
    ], PowerupConfigResource.prototype, "max_price", void 0);
    PowerupConfigResource = tslib.__decorate([
        antelope.Struct.type('powerup_config_resource')
    ], PowerupConfigResource);
    Types.PowerupConfigResource = PowerupConfigResource;
    let PowerupConfig = class PowerupConfig extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(PowerupConfigResource)
    ], PowerupConfig.prototype, "net", void 0);
    tslib.__decorate([
        antelope.Struct.field(PowerupConfigResource)
    ], PowerupConfig.prototype, "cpu", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32, { optional: true })
    ], PowerupConfig.prototype, "powerup_days", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset, { optional: true })
    ], PowerupConfig.prototype, "min_powerup_fee", void 0);
    PowerupConfig = tslib.__decorate([
        antelope.Struct.type('powerup_config')
    ], PowerupConfig);
    Types.PowerupConfig = PowerupConfig;
    let Cfgpowerup = class Cfgpowerup extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(PowerupConfig)
    ], Cfgpowerup.prototype, "args", void 0);
    Cfgpowerup = tslib.__decorate([
        antelope.Struct.type('cfgpowerup')
    ], Cfgpowerup);
    Types.Cfgpowerup = Cfgpowerup;
    let Claimrewards = class Claimrewards extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Claimrewards.prototype, "owner", void 0);
    Claimrewards = tslib.__decorate([
        antelope.Struct.type('claimrewards')
    ], Claimrewards);
    Types.Claimrewards = Claimrewards;
    let Closerex = class Closerex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Closerex.prototype, "owner", void 0);
    Closerex = tslib.__decorate([
        antelope.Struct.type('closerex')
    ], Closerex);
    Types.Closerex = Closerex;
    let Cnclrexorder = class Cnclrexorder extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Cnclrexorder.prototype, "owner", void 0);
    Cnclrexorder = tslib.__decorate([
        antelope.Struct.type('cnclrexorder')
    ], Cnclrexorder);
    Types.Cnclrexorder = Cnclrexorder;
    let Connector = class Connector extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Connector.prototype, "balance", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], Connector.prototype, "weight", void 0);
    Connector = tslib.__decorate([
        antelope.Struct.type('connector')
    ], Connector);
    Types.Connector = Connector;
    let Consolidate = class Consolidate extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Consolidate.prototype, "owner", void 0);
    Consolidate = tslib.__decorate([
        antelope.Struct.type('consolidate')
    ], Consolidate);
    Types.Consolidate = Consolidate;
    let Defcpuloan = class Defcpuloan extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Defcpuloan.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], Defcpuloan.prototype, "loan_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Defcpuloan.prototype, "amount", void 0);
    Defcpuloan = tslib.__decorate([
        antelope.Struct.type('defcpuloan')
    ], Defcpuloan);
    Types.Defcpuloan = Defcpuloan;
    let Defnetloan = class Defnetloan extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Defnetloan.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], Defnetloan.prototype, "loan_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Defnetloan.prototype, "amount", void 0);
    Defnetloan = tslib.__decorate([
        antelope.Struct.type('defnetloan')
    ], Defnetloan);
    Types.Defnetloan = Defnetloan;
    let Delegatebw = class Delegatebw extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Delegatebw.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Delegatebw.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Delegatebw.prototype, "stake_net_quantity", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Delegatebw.prototype, "stake_cpu_quantity", void 0);
    tslib.__decorate([
        antelope.Struct.field('bool')
    ], Delegatebw.prototype, "transfer", void 0);
    Delegatebw = tslib.__decorate([
        antelope.Struct.type('delegatebw')
    ], Delegatebw);
    Types.Delegatebw = Delegatebw;
    let DelegatedBandwidth = class DelegatedBandwidth extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], DelegatedBandwidth.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], DelegatedBandwidth.prototype, "to", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], DelegatedBandwidth.prototype, "net_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], DelegatedBandwidth.prototype, "cpu_weight", void 0);
    DelegatedBandwidth = tslib.__decorate([
        antelope.Struct.type('delegated_bandwidth')
    ], DelegatedBandwidth);
    Types.DelegatedBandwidth = DelegatedBandwidth;
    let Deleteauth = class Deleteauth extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Deleteauth.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Deleteauth.prototype, "permission", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { optional: true })
    ], Deleteauth.prototype, "authorized_by", void 0);
    Deleteauth = tslib.__decorate([
        antelope.Struct.type('deleteauth')
    ], Deleteauth);
    Types.Deleteauth = Deleteauth;
    let Deposit = class Deposit extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Deposit.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Deposit.prototype, "amount", void 0);
    Deposit = tslib.__decorate([
        antelope.Struct.type('deposit')
    ], Deposit);
    Types.Deposit = Deposit;
    let EosioGlobalState = class EosioGlobalState extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], EosioGlobalState.prototype, "max_ram_size", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], EosioGlobalState.prototype, "total_ram_bytes_reserved", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState.prototype, "total_ram_stake", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.BlockTimestamp)
    ], EosioGlobalState.prototype, "last_producer_schedule_update", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], EosioGlobalState.prototype, "last_pervote_bucket_fill", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState.prototype, "pervote_bucket", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState.prototype, "perblock_bucket", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], EosioGlobalState.prototype, "total_unpaid_blocks", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState.prototype, "total_activated_stake", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], EosioGlobalState.prototype, "thresh_activated_stake_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], EosioGlobalState.prototype, "last_producer_schedule_size", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], EosioGlobalState.prototype, "total_producer_vote_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.BlockTimestamp)
    ], EosioGlobalState.prototype, "last_name_close", void 0);
    EosioGlobalState = tslib.__decorate([
        antelope.Struct.type('eosio_global_state')
    ], EosioGlobalState);
    Types.EosioGlobalState = EosioGlobalState;
    let EosioGlobalState2 = class EosioGlobalState2 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], EosioGlobalState2.prototype, "new_ram_per_block", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.BlockTimestamp)
    ], EosioGlobalState2.prototype, "last_ram_increase", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.BlockTimestamp)
    ], EosioGlobalState2.prototype, "last_block_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], EosioGlobalState2.prototype, "total_producer_votepay_share", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], EosioGlobalState2.prototype, "revision", void 0);
    EosioGlobalState2 = tslib.__decorate([
        antelope.Struct.type('eosio_global_state2')
    ], EosioGlobalState2);
    Types.EosioGlobalState2 = EosioGlobalState2;
    let EosioGlobalState3 = class EosioGlobalState3 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], EosioGlobalState3.prototype, "last_vpay_state_update", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], EosioGlobalState3.prototype, "total_vpay_share_change_rate", void 0);
    EosioGlobalState3 = tslib.__decorate([
        antelope.Struct.type('eosio_global_state3')
    ], EosioGlobalState3);
    Types.EosioGlobalState3 = EosioGlobalState3;
    let EosioGlobalState4 = class EosioGlobalState4 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], EosioGlobalState4.prototype, "continuous_rate", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState4.prototype, "inflation_pay_factor", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], EosioGlobalState4.prototype, "votepay_factor", void 0);
    EosioGlobalState4 = tslib.__decorate([
        antelope.Struct.type('eosio_global_state4')
    ], EosioGlobalState4);
    Types.EosioGlobalState4 = EosioGlobalState4;
    let ExchangeState = class ExchangeState extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], ExchangeState.prototype, "supply", void 0);
    tslib.__decorate([
        antelope.Struct.field(Connector)
    ], ExchangeState.prototype, "base", void 0);
    tslib.__decorate([
        antelope.Struct.field(Connector)
    ], ExchangeState.prototype, "quote", void 0);
    ExchangeState = tslib.__decorate([
        antelope.Struct.type('exchange_state')
    ], ExchangeState);
    Types.ExchangeState = ExchangeState;
    let Fundcpuloan = class Fundcpuloan extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Fundcpuloan.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], Fundcpuloan.prototype, "loan_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Fundcpuloan.prototype, "payment", void 0);
    Fundcpuloan = tslib.__decorate([
        antelope.Struct.type('fundcpuloan')
    ], Fundcpuloan);
    Types.Fundcpuloan = Fundcpuloan;
    let Fundnetloan = class Fundnetloan extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Fundnetloan.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], Fundnetloan.prototype, "loan_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Fundnetloan.prototype, "payment", void 0);
    Fundnetloan = tslib.__decorate([
        antelope.Struct.type('fundnetloan')
    ], Fundnetloan);
    Types.Fundnetloan = Fundnetloan;
    let Init = class Init extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.VarUInt)
    ], Init.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset.Symbol)
    ], Init.prototype, "core", void 0);
    Init = tslib.__decorate([
        antelope.Struct.type('init')
    ], Init);
    Types.Init = Init;
    let Limitauthchg = class Limitauthchg extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Limitauthchg.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], Limitauthchg.prototype, "allow_perms", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], Limitauthchg.prototype, "disallow_perms", void 0);
    Limitauthchg = tslib.__decorate([
        antelope.Struct.type('limitauthchg')
    ], Limitauthchg);
    Types.Limitauthchg = Limitauthchg;
    let Linkauth = class Linkauth extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Linkauth.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Linkauth.prototype, "code", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Linkauth.prototype, "type", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Linkauth.prototype, "requirement", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { optional: true })
    ], Linkauth.prototype, "authorized_by", void 0);
    Linkauth = tslib.__decorate([
        antelope.Struct.type('linkauth')
    ], Linkauth);
    Types.Linkauth = Linkauth;
    let Mvfrsavings = class Mvfrsavings extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Mvfrsavings.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Mvfrsavings.prototype, "rex", void 0);
    Mvfrsavings = tslib.__decorate([
        antelope.Struct.type('mvfrsavings')
    ], Mvfrsavings);
    Types.Mvfrsavings = Mvfrsavings;
    let Mvtosavings = class Mvtosavings extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Mvtosavings.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Mvtosavings.prototype, "rex", void 0);
    Mvtosavings = tslib.__decorate([
        antelope.Struct.type('mvtosavings')
    ], Mvtosavings);
    Types.Mvtosavings = Mvtosavings;
    let NameBid = class NameBid extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], NameBid.prototype, "newname", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], NameBid.prototype, "high_bidder", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], NameBid.prototype, "high_bid", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], NameBid.prototype, "last_bid_time", void 0);
    NameBid = tslib.__decorate([
        antelope.Struct.type('name_bid')
    ], NameBid);
    Types.NameBid = NameBid;
    let Newaccount = class Newaccount extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Newaccount.prototype, "creator", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Newaccount.prototype, "name", void 0);
    tslib.__decorate([
        antelope.Struct.field(Authority)
    ], Newaccount.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(Authority)
    ], Newaccount.prototype, "active", void 0);
    Newaccount = tslib.__decorate([
        antelope.Struct.type('newaccount')
    ], Newaccount);
    Types.Newaccount = Newaccount;
    let Onblock = class Onblock extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(BlockHeader)
    ], Onblock.prototype, "header", void 0);
    Onblock = tslib.__decorate([
        antelope.Struct.type('onblock')
    ], Onblock);
    Types.Onblock = Onblock;
    let Onerror = class Onerror extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt128)
    ], Onerror.prototype, "sender_id", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Bytes)
    ], Onerror.prototype, "sent_trx", void 0);
    Onerror = tslib.__decorate([
        antelope.Struct.type('onerror')
    ], Onerror);
    Types.Onerror = Onerror;
    let PairTimePointSecInt64 = class PairTimePointSecInt64 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], PairTimePointSecInt64.prototype, "first", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PairTimePointSecInt64.prototype, "second", void 0);
    PairTimePointSecInt64 = tslib.__decorate([
        antelope.Struct.type('pair_time_point_sec_int64')
    ], PairTimePointSecInt64);
    Types.PairTimePointSecInt64 = PairTimePointSecInt64;
    let Powerup = class Powerup extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Powerup.prototype, "payer", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Powerup.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], Powerup.prototype, "days", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Powerup.prototype, "net_frac", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Powerup.prototype, "cpu_frac", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Powerup.prototype, "max_payment", void 0);
    Powerup = tslib.__decorate([
        antelope.Struct.type('powerup')
    ], Powerup);
    Types.Powerup = Powerup;
    let PowerupOrder = class PowerupOrder extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], PowerupOrder.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], PowerupOrder.prototype, "id", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], PowerupOrder.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupOrder.prototype, "net_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupOrder.prototype, "cpu_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], PowerupOrder.prototype, "expires", void 0);
    PowerupOrder = tslib.__decorate([
        antelope.Struct.type('powerup_order')
    ], PowerupOrder);
    Types.PowerupOrder = PowerupOrder;
    let PowerupStateResource = class PowerupStateResource extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], PowerupStateResource.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "weight_ratio", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "assumed_stake_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "initial_weight_ratio", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "target_weight_ratio", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], PowerupStateResource.prototype, "initial_timestamp", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], PowerupStateResource.prototype, "target_timestamp", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], PowerupStateResource.prototype, "exponent", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], PowerupStateResource.prototype, "decay_secs", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], PowerupStateResource.prototype, "min_price", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], PowerupStateResource.prototype, "max_price", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "utilization", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], PowerupStateResource.prototype, "adjusted_utilization", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], PowerupStateResource.prototype, "utilization_timestamp", void 0);
    PowerupStateResource = tslib.__decorate([
        antelope.Struct.type('powerup_state_resource')
    ], PowerupStateResource);
    Types.PowerupStateResource = PowerupStateResource;
    let PowerupState = class PowerupState extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], PowerupState.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(PowerupStateResource)
    ], PowerupState.prototype, "net", void 0);
    tslib.__decorate([
        antelope.Struct.field(PowerupStateResource)
    ], PowerupState.prototype, "cpu", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], PowerupState.prototype, "powerup_days", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], PowerupState.prototype, "min_powerup_fee", void 0);
    PowerupState = tslib.__decorate([
        antelope.Struct.type('powerup_state')
    ], PowerupState);
    Types.PowerupState = PowerupState;
    let Powerupexec = class Powerupexec extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Powerupexec.prototype, "user", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], Powerupexec.prototype, "max", void 0);
    Powerupexec = tslib.__decorate([
        antelope.Struct.type('powerupexec')
    ], Powerupexec);
    Types.Powerupexec = Powerupexec;
    let ProducerInfo = class ProducerInfo extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], ProducerInfo.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], ProducerInfo.prototype, "total_votes", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.PublicKey)
    ], ProducerInfo.prototype, "producer_key", void 0);
    tslib.__decorate([
        antelope.Struct.field('bool')
    ], ProducerInfo.prototype, "is_active", void 0);
    tslib.__decorate([
        antelope.Struct.field('string')
    ], ProducerInfo.prototype, "url", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], ProducerInfo.prototype, "unpaid_blocks", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], ProducerInfo.prototype, "last_claim_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], ProducerInfo.prototype, "location", void 0);
    tslib.__decorate([
        antelope.Struct.field(BlockSigningAuthorityV0, { optional: true })
    ], ProducerInfo.prototype, "producer_authority", void 0);
    ProducerInfo = tslib.__decorate([
        antelope.Struct.type('producer_info')
    ], ProducerInfo);
    Types.ProducerInfo = ProducerInfo;
    let ProducerInfo2 = class ProducerInfo2 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], ProducerInfo2.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], ProducerInfo2.prototype, "votepay_share", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], ProducerInfo2.prototype, "last_votepay_share_update", void 0);
    ProducerInfo2 = tslib.__decorate([
        antelope.Struct.type('producer_info2')
    ], ProducerInfo2);
    Types.ProducerInfo2 = ProducerInfo2;
    let Refund = class Refund extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Refund.prototype, "owner", void 0);
    Refund = tslib.__decorate([
        antelope.Struct.type('refund')
    ], Refund);
    Types.Refund = Refund;
    let RefundRequest = class RefundRequest extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RefundRequest.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], RefundRequest.prototype, "request_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RefundRequest.prototype, "net_amount", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RefundRequest.prototype, "cpu_amount", void 0);
    RefundRequest = tslib.__decorate([
        antelope.Struct.type('refund_request')
    ], RefundRequest);
    Types.RefundRequest = RefundRequest;
    let Regproducer = class Regproducer extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Regproducer.prototype, "producer", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.PublicKey)
    ], Regproducer.prototype, "producer_key", void 0);
    tslib.__decorate([
        antelope.Struct.field('string')
    ], Regproducer.prototype, "url", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], Regproducer.prototype, "location", void 0);
    Regproducer = tslib.__decorate([
        antelope.Struct.type('regproducer')
    ], Regproducer);
    Types.Regproducer = Regproducer;
    let Regproducer2 = class Regproducer2 extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Regproducer2.prototype, "producer", void 0);
    tslib.__decorate([
        antelope.Struct.field(BlockSigningAuthorityV0)
    ], Regproducer2.prototype, "producer_authority", void 0);
    tslib.__decorate([
        antelope.Struct.field('string')
    ], Regproducer2.prototype, "url", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], Regproducer2.prototype, "location", void 0);
    Regproducer2 = tslib.__decorate([
        antelope.Struct.type('regproducer2')
    ], Regproducer2);
    Types.Regproducer2 = Regproducer2;
    let Regproxy = class Regproxy extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Regproxy.prototype, "proxy", void 0);
    tslib.__decorate([
        antelope.Struct.field('bool')
    ], Regproxy.prototype, "isproxy", void 0);
    Regproxy = tslib.__decorate([
        antelope.Struct.type('regproxy')
    ], Regproxy);
    Types.Regproxy = Regproxy;
    let Rentcpu = class Rentcpu extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rentcpu.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rentcpu.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Rentcpu.prototype, "loan_payment", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Rentcpu.prototype, "loan_fund", void 0);
    Rentcpu = tslib.__decorate([
        antelope.Struct.type('rentcpu')
    ], Rentcpu);
    Types.Rentcpu = Rentcpu;
    let Rentnet = class Rentnet extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rentnet.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rentnet.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Rentnet.prototype, "loan_payment", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Rentnet.prototype, "loan_fund", void 0);
    Rentnet = tslib.__decorate([
        antelope.Struct.type('rentnet')
    ], Rentnet);
    Types.Rentnet = Rentnet;
    let RexBalance = class RexBalance extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexBalance.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RexBalance.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexBalance.prototype, "vote_stake", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexBalance.prototype, "rex_balance", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], RexBalance.prototype, "matured_rex", void 0);
    tslib.__decorate([
        antelope.Struct.field(PairTimePointSecInt64, { array: true })
    ], RexBalance.prototype, "rex_maturities", void 0);
    RexBalance = tslib.__decorate([
        antelope.Struct.type('rex_balance')
    ], RexBalance);
    Types.RexBalance = RexBalance;
    let RexFund = class RexFund extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexFund.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RexFund.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexFund.prototype, "balance", void 0);
    RexFund = tslib.__decorate([
        antelope.Struct.type('rex_fund')
    ], RexFund);
    Types.RexFund = RexFund;
    let RexLoan = class RexLoan extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexLoan.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RexLoan.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RexLoan.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexLoan.prototype, "payment", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexLoan.prototype, "balance", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexLoan.prototype, "total_staked", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], RexLoan.prototype, "loan_num", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], RexLoan.prototype, "expiration", void 0);
    RexLoan = tslib.__decorate([
        antelope.Struct.type('rex_loan')
    ], RexLoan);
    Types.RexLoan = RexLoan;
    let RexOrder = class RexOrder extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexOrder.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], RexOrder.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexOrder.prototype, "rex_requested", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexOrder.prototype, "proceeds", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexOrder.prototype, "stake_change", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePoint)
    ], RexOrder.prototype, "order_time", void 0);
    tslib.__decorate([
        antelope.Struct.field('bool')
    ], RexOrder.prototype, "is_open", void 0);
    RexOrder = tslib.__decorate([
        antelope.Struct.type('rex_order')
    ], RexOrder);
    Types.RexOrder = RexOrder;
    let RexPool = class RexPool extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexPool.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "total_lent", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "total_unlent", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "total_rent", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "total_lendable", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "total_rex", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], RexPool.prototype, "namebid_proceeds", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], RexPool.prototype, "loan_num", void 0);
    RexPool = tslib.__decorate([
        antelope.Struct.type('rex_pool')
    ], RexPool);
    Types.RexPool = RexPool;
    let RexReturnBuckets = class RexReturnBuckets extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexReturnBuckets.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(PairTimePointSecInt64, { array: true })
    ], RexReturnBuckets.prototype, "return_buckets", void 0);
    RexReturnBuckets = tslib.__decorate([
        antelope.Struct.type('rex_return_buckets')
    ], RexReturnBuckets);
    Types.RexReturnBuckets = RexReturnBuckets;
    let RexReturnPool = class RexReturnPool extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], RexReturnPool.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], RexReturnPool.prototype, "last_dist_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], RexReturnPool.prototype, "pending_bucket_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.TimePointSec)
    ], RexReturnPool.prototype, "oldest_bucket_time", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], RexReturnPool.prototype, "pending_bucket_proceeds", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], RexReturnPool.prototype, "current_rate_of_increase", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], RexReturnPool.prototype, "proceeds", void 0);
    RexReturnPool = tslib.__decorate([
        antelope.Struct.type('rex_return_pool')
    ], RexReturnPool);
    Types.RexReturnPool = RexReturnPool;
    let Rexexec = class Rexexec extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rexexec.prototype, "user", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], Rexexec.prototype, "max", void 0);
    Rexexec = tslib.__decorate([
        antelope.Struct.type('rexexec')
    ], Rexexec);
    Types.Rexexec = Rexexec;
    let Rmvproducer = class Rmvproducer extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Rmvproducer.prototype, "producer", void 0);
    Rmvproducer = tslib.__decorate([
        antelope.Struct.type('rmvproducer')
    ], Rmvproducer);
    Types.Rmvproducer = Rmvproducer;
    let Sellram = class Sellram extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Sellram.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Sellram.prototype, "bytes", void 0);
    Sellram = tslib.__decorate([
        antelope.Struct.type('sellram')
    ], Sellram);
    Types.Sellram = Sellram;
    let Sellrex = class Sellrex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Sellrex.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Sellrex.prototype, "rex", void 0);
    Sellrex = tslib.__decorate([
        antelope.Struct.type('sellrex')
    ], Sellrex);
    Types.Sellrex = Sellrex;
    let Setabi = class Setabi extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setabi.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Bytes)
    ], Setabi.prototype, "abi", void 0);
    tslib.__decorate([
        antelope.Struct.field('string', { optional: true })
    ], Setabi.prototype, "memo", void 0);
    Setabi = tslib.__decorate([
        antelope.Struct.type('setabi')
    ], Setabi);
    Types.Setabi = Setabi;
    let Setacctcpu = class Setacctcpu extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setacctcpu.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], Setacctcpu.prototype, "cpu_weight", void 0);
    Setacctcpu = tslib.__decorate([
        antelope.Struct.type('setacctcpu')
    ], Setacctcpu);
    Types.Setacctcpu = Setacctcpu;
    let Setacctnet = class Setacctnet extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setacctnet.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], Setacctnet.prototype, "net_weight", void 0);
    Setacctnet = tslib.__decorate([
        antelope.Struct.type('setacctnet')
    ], Setacctnet);
    Types.Setacctnet = Setacctnet;
    let Setacctram = class Setacctram extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setacctram.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64, { optional: true })
    ], Setacctram.prototype, "ram_bytes", void 0);
    Setacctram = tslib.__decorate([
        antelope.Struct.type('setacctram')
    ], Setacctram);
    Types.Setacctram = Setacctram;
    let Setalimits = class Setalimits extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setalimits.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setalimits.prototype, "ram_bytes", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setalimits.prototype, "net_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setalimits.prototype, "cpu_weight", void 0);
    Setalimits = tslib.__decorate([
        antelope.Struct.type('setalimits')
    ], Setalimits);
    Types.Setalimits = Setalimits;
    let Setcode = class Setcode extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setcode.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], Setcode.prototype, "vmtype", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], Setcode.prototype, "vmversion", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Bytes)
    ], Setcode.prototype, "code", void 0);
    tslib.__decorate([
        antelope.Struct.field('string', { optional: true })
    ], Setcode.prototype, "memo", void 0);
    Setcode = tslib.__decorate([
        antelope.Struct.type('setcode')
    ], Setcode);
    Types.Setcode = Setcode;
    let Setinflation = class Setinflation extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setinflation.prototype, "annual_rate", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setinflation.prototype, "inflation_pay_factor", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], Setinflation.prototype, "votepay_factor", void 0);
    Setinflation = tslib.__decorate([
        antelope.Struct.type('setinflation')
    ], Setinflation);
    Types.Setinflation = Setinflation;
    let Setparams = class Setparams extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(BlockchainParametersV1)
    ], Setparams.prototype, "params", void 0);
    Setparams = tslib.__decorate([
        antelope.Struct.type('setparams')
    ], Setparams);
    Types.Setparams = Setparams;
    let Setpriv = class Setpriv extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Setpriv.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], Setpriv.prototype, "is_priv", void 0);
    Setpriv = tslib.__decorate([
        antelope.Struct.type('setpriv')
    ], Setpriv);
    Types.Setpriv = Setpriv;
    let Setram = class Setram extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt64)
    ], Setram.prototype, "max_ram_size", void 0);
    Setram = tslib.__decorate([
        antelope.Struct.type('setram')
    ], Setram);
    Types.Setram = Setram;
    let Setramrate = class Setramrate extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt16)
    ], Setramrate.prototype, "bytes_per_block", void 0);
    Setramrate = tslib.__decorate([
        antelope.Struct.type('setramrate')
    ], Setramrate);
    Types.Setramrate = Setramrate;
    let Setrex = class Setrex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Setrex.prototype, "balance", void 0);
    Setrex = tslib.__decorate([
        antelope.Struct.type('setrex')
    ], Setrex);
    Types.Setrex = Setrex;
    let Undelegatebw = class Undelegatebw extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Undelegatebw.prototype, "from", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Undelegatebw.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Undelegatebw.prototype, "unstake_net_quantity", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Undelegatebw.prototype, "unstake_cpu_quantity", void 0);
    Undelegatebw = tslib.__decorate([
        antelope.Struct.type('undelegatebw')
    ], Undelegatebw);
    Types.Undelegatebw = Undelegatebw;
    let Unlinkauth = class Unlinkauth extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unlinkauth.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unlinkauth.prototype, "code", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unlinkauth.prototype, "type", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { optional: true })
    ], Unlinkauth.prototype, "authorized_by", void 0);
    Unlinkauth = tslib.__decorate([
        antelope.Struct.type('unlinkauth')
    ], Unlinkauth);
    Types.Unlinkauth = Unlinkauth;
    let Unregprod = class Unregprod extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unregprod.prototype, "producer", void 0);
    Unregprod = tslib.__decorate([
        antelope.Struct.type('unregprod')
    ], Unregprod);
    Types.Unregprod = Unregprod;
    let Unstaketorex = class Unstaketorex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unstaketorex.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Unstaketorex.prototype, "receiver", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Unstaketorex.prototype, "from_net", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Unstaketorex.prototype, "from_cpu", void 0);
    Unstaketorex = tslib.__decorate([
        antelope.Struct.type('unstaketorex')
    ], Unstaketorex);
    Types.Unstaketorex = Unstaketorex;
    let Updateauth = class Updateauth extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Updateauth.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Updateauth.prototype, "permission", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Updateauth.prototype, "parent", void 0);
    tslib.__decorate([
        antelope.Struct.field(Authority)
    ], Updateauth.prototype, "auth", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { optional: true })
    ], Updateauth.prototype, "authorized_by", void 0);
    Updateauth = tslib.__decorate([
        antelope.Struct.type('updateauth')
    ], Updateauth);
    Types.Updateauth = Updateauth;
    let Updaterex = class Updaterex extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Updaterex.prototype, "owner", void 0);
    Updaterex = tslib.__decorate([
        antelope.Struct.type('updaterex')
    ], Updaterex);
    Types.Updaterex = Updaterex;
    let Updtrevision = class Updtrevision extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], Updtrevision.prototype, "revision", void 0);
    Updtrevision = tslib.__decorate([
        antelope.Struct.type('updtrevision')
    ], Updtrevision);
    Types.Updtrevision = Updtrevision;
    let UserResources = class UserResources extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], UserResources.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], UserResources.prototype, "net_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], UserResources.prototype, "cpu_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], UserResources.prototype, "ram_bytes", void 0);
    UserResources = tslib.__decorate([
        antelope.Struct.type('user_resources')
    ], UserResources);
    Types.UserResources = UserResources;
    let Voteproducer = class Voteproducer extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Voteproducer.prototype, "voter", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Voteproducer.prototype, "proxy", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], Voteproducer.prototype, "producers", void 0);
    Voteproducer = tslib.__decorate([
        antelope.Struct.type('voteproducer')
    ], Voteproducer);
    Types.Voteproducer = Voteproducer;
    let VoterInfo = class VoterInfo extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], VoterInfo.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], VoterInfo.prototype, "proxy", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], VoterInfo.prototype, "producers", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Int64)
    ], VoterInfo.prototype, "staked", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], VoterInfo.prototype, "last_vote_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Float64)
    ], VoterInfo.prototype, "proxied_vote_weight", void 0);
    tslib.__decorate([
        antelope.Struct.field('bool')
    ], VoterInfo.prototype, "is_proxy", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], VoterInfo.prototype, "flags1", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt32)
    ], VoterInfo.prototype, "reserved2", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], VoterInfo.prototype, "reserved3", void 0);
    VoterInfo = tslib.__decorate([
        antelope.Struct.type('voter_info')
    ], VoterInfo);
    Types.VoterInfo = VoterInfo;
    let Voteupdate = class Voteupdate extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Voteupdate.prototype, "voter_name", void 0);
    Voteupdate = tslib.__decorate([
        antelope.Struct.type('voteupdate')
    ], Voteupdate);
    Types.Voteupdate = Voteupdate;
    let Wasmcfg = class Wasmcfg extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Wasmcfg.prototype, "settings", void 0);
    Wasmcfg = tslib.__decorate([
        antelope.Struct.type('wasmcfg')
    ], Wasmcfg);
    Types.Wasmcfg = Wasmcfg;
    let Withdraw = class Withdraw extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], Withdraw.prototype, "owner", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Asset)
    ], Withdraw.prototype, "amount", void 0);
    Withdraw = tslib.__decorate([
        antelope.Struct.type('withdraw')
    ], Withdraw);
    Types.Withdraw = Withdraw;
    let LimitAuthChange = class LimitAuthChange extends antelope.Struct {
    };
    tslib.__decorate([
        antelope.Struct.field(antelope.UInt8)
    ], LimitAuthChange.prototype, "version", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name)
    ], LimitAuthChange.prototype, "account", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], LimitAuthChange.prototype, "allow_perms", void 0);
    tslib.__decorate([
        antelope.Struct.field(antelope.Name, { array: true })
    ], LimitAuthChange.prototype, "disallow_perms", void 0);
    LimitAuthChange = tslib.__decorate([
        antelope.Struct.type('limit_auth_change')
    ], LimitAuthChange);
    Types.LimitAuthChange = LimitAuthChange;
})(Types || (Types = {}));
const TableMap = {
    abihash: Types.AbiHash,
    bidrefunds: Types.BidRefund,
    blockinfo: Types.BlockInfoRecord,
    cpuloan: Types.RexLoan,
    delband: Types.DelegatedBandwidth,
    global: Types.EosioGlobalState,
    global2: Types.EosioGlobalState2,
    global3: Types.EosioGlobalState3,
    global4: Types.EosioGlobalState4,
    namebids: Types.NameBid,
    netloan: Types.RexLoan,
    'powup.order': Types.PowerupOrder,
    'powup.state': Types.PowerupState,
    producers: Types.ProducerInfo,
    producers2: Types.ProducerInfo2,
    rammarket: Types.ExchangeState,
    refunds: Types.RefundRequest,
    retbuckets: Types.RexReturnBuckets,
    rexbal: Types.RexBalance,
    rexfund: Types.RexFund,
    rexpool: Types.RexPool,
    rexqueue: Types.RexOrder,
    rexretpool: Types.RexReturnPool,
    userres: Types.UserResources,
    voters: Types.VoterInfo,
    limitauthchg: Types.LimitAuthChange,
};

var eosio = /*#__PURE__*/Object.freeze({
    __proto__: null,
    Contract: Contract,
    get Types () { return Types; },
    abi: abi,
    abiBlob: abiBlob
});

class Resource {
    constructor(resource, data) {
        this.resource = resource;
        this.data = data;
        switch (resource) {
            case 'cpu': {
                this.available = this.data.cpu_limit.available;
                this.current_used = this.data.cpu_limit.current_used;
                this.used = this.data.cpu_limit.used;
                this.max = this.data.cpu_limit.max;
                this.weight = this.data.cpu_weight;
                break;
            }
            case 'net': {
                this.available = this.data.net_limit.available;
                this.current_used = this.data.net_limit.current_used;
                this.used = this.data.net_limit.used;
                this.max = this.data.net_limit.max;
                this.weight = this.data.net_weight;
                break;
            }
            case 'ram': {
                this.available = this.data.ram_quota.subtracting(this.data.ram_usage);
                this.used = antelope.Int64.from(this.data.ram_usage);
                this.max = this.data.ram_quota;
                break;
            }
            default: {
                throw new Error(`Unknown resource type (${resource}).`);
            }
        }
    }
    toJSON() {
        return {
            resource: this.resource,
            available: this.available,
            current_used: this.current_used ? this.current_used : this.current_used,
            used: this.used,
            max: this.max,
            weight: this.weight ? this.weight : undefined,
        };
    }
}

class Account {
    constructor(args) {
        this.data = args.data;
        if (args.contract) {
            this.systemContract = args.contract;
        }
        else {
            this.systemContract = new Contract({ client: args.client });
        }
        this.client = args.client;
        this.token = new token.Token({ client: args.client });
    }
    get accountName() {
        return antelope.Name.from(this.data.account_name);
    }
    get systemToken() {
        return antelope.Asset.Symbol.from(this.data.total_resources.cpu_weight.symbol);
    }
    balance(symbol, tokenContract) {
        return this.token.balance(this.accountName, symbol, tokenContract);
    }
    permission(permissionName) {
        const permission = this.data.permissions.find((permission) => permission.perm_name.equals(permissionName));
        if (!permission) {
            throw new Error(`Permission ${permissionName} does not exist on account ${this.accountName}.`);
        }
        return exports.Permission.from(permission);
    }
    get permissions() {
        return this.data.permissions.map((permission) => exports.Permission.from(permission));
    }
    resource(resourceType) {
        return new Resource(resourceType, this.data);
    }
    // TODO: Refactor once resources library is updated
    resources(sampleAccount) {
        // Returns an instance of the @wharfkit/resources library
        //   configured for this blockchain/account
        return new resources.Resources({
            api: this.client,
            sampleAccount: sampleAccount ? String(sampleAccount) : undefined,
            symbol: this.data.core_liquid_balance
                ? String(this.data.core_liquid_balance.symbol)
                : undefined,
        });
    }
    setPermission(permission) {
        return this.systemContract.action('updateauth', {
            account: this.accountName,
            auth: permission.required_auth,
            authorized_by: '',
            parent: permission.parent,
            permission: permission.perm_name,
        });
    }
    removePermission(permissionName) {
        return this.systemContract.action('deleteauth', {
            account: this.accountName,
            authorized_by: '',
            permission: permissionName,
        });
    }
    linkauth(contract, action, requiredPermission) {
        return this.systemContract.action('linkauth', {
            account: this.accountName,
            code: contract,
            type: action,
            requirement: requiredPermission,
            authorized_by: '',
        });
    }
    unlinkauth(contract, action) {
        return this.systemContract.action('unlinkauth', {
            account: this.accountName,
            code: contract,
            type: action,
            authorized_by: '',
        });
    }
    buyRam(amount, options) {
        let receiver = this.accountName;
        if (options && options.receiver) {
            receiver = antelope.Name.from(options.receiver);
        }
        return this.systemContract.action('buyram', {
            payer: this.accountName,
            quant: amount,
            receiver,
        });
    }
    buyRamBytes(bytes, options) {
        let receiver = this.accountName;
        if (options && options.receiver) {
            receiver = antelope.Name.from(options.receiver);
        }
        return this.systemContract.action('buyrambytes', {
            bytes,
            payer: this.accountName,
            receiver,
        });
    }
    sellRam(bytes) {
        return this.systemContract.action('sellram', {
            account: this.accountName,
            bytes,
        });
    }
    delegate(value) {
        return this.systemContract.action('delegatebw', {
            from: value.from || this.accountName,
            receiver: value.receiver || this.accountName,
            stake_cpu_quantity: value.cpu || antelope.Asset.fromUnits(0, this.systemToken),
            stake_net_quantity: value.net || antelope.Asset.fromUnits(0, this.systemToken),
            transfer: value.transfer !== undefined ? value.transfer : false,
        });
    }
    undelegate(value) {
        return this.systemContract.action('undelegatebw', {
            from: value.from || this.accountName,
            receiver: value.receiver || this.accountName,
            unstake_cpu_quantity: value.cpu || antelope.Asset.fromUnits(0, this.systemToken),
            unstake_net_quantity: value.net || antelope.Asset.fromUnits(0, this.systemToken),
        });
    }
}

class AccountKit {
    constructor(chain, options) {
        this.chain = chain;
        this.contract = options === null || options === void 0 ? void 0 : options.contract;
        this.client = (options === null || options === void 0 ? void 0 : options.client) || new antelope.APIClient({ url: this.chain.url });
    }
    load(accountName) {
        return tslib.__awaiter(this, void 0, void 0, function* () {
            const data = yield this.client.v1.chain.get_account(accountName, this.chain.accountDataType);
            return new Account({
                client: this.client,
                contract: this.contract,
                data: data,
            });
        });
    }
}

exports.Account = Account;
exports.AccountKit = AccountKit;
exports.Resource = Resource;
exports.SystemContract = eosio;
//# sourceMappingURL=account.js.map
